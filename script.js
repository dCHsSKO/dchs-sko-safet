...
const themeBtn=document.getElementById('themeBtn');const themes=['dark','light','contrast'];let theme=localStorage.getItem('dchsTheme')||'dark';if(!themes.includes(theme))theme='dark';
function applyTheme(){document.documentElement.dataset.theme=theme;document.documentElement.style.colorScheme=theme==='dark'?'dark':'light';const label=window.I18N?.[document.documentElement.lang]?.themeLabels?.[theme]||({dark:'ТЁМНАЯ',light:'СВЕТЛАЯ',contrast:'КОНТРАСТ'}[theme]);themeBtn.querySelector('span').textContent=label;localStorage.setItem('dchsTheme',theme)}
themeBtn.addEventListener('click',()=>{theme=themes[(themes.indexOf(theme)+1)%themes.length];applyTheme()});applyTheme();
